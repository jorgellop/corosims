...
if __name__ == '__main__':
    ...
    if not flag_loop:
        ...
        # Example input image as a 2D NumPy array
        image = ni_im
        
        #%% Create figure and axis
        fig, ax = plt.subplots()
        
        # Plot the image with a nice color scheme
        cmap = plt.get_cmap('viridis')  # Choose a colormap (e.g., 'viridis')
        norm = Normalize(vmin=np.min(image), vmax=np.max(image))  # Normalize the data
        ...
        # Use LaTeX font
        ax.set_xlabel(r'$\mathrm{X\ Label}$')
        ax.set_ylabel(r'$\mathrm{Y\ Label}$')
        
        # Set title with LaTeX-like font
        ax.set_title(r'$\mathrm{Example\ Image}$')
